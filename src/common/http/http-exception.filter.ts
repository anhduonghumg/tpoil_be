...
export class HttpExceptionFilter implements ExceptionFilter {
        ...
        // 2) HttpException (bao gồm UnauthorizedException, BadRequestException, ...)
            ...
            if (typeof body === 'string') {
                // Không có payload chi tiết -> map code theo status
                message = body
                code =
                    status === 404
                        ? ErrCode.NOT_FOUND
                ...
                // ƯU TIÊN dùng code đến từ exception nếu có (vd: 'SESSION_EXPIRED')
                const incomingCode = typeof r.code === 'string' && r.code.trim() ? r.code.trim() : undefined

                if (Array.isArray(r.message)) {
                    code = incomingCode ?? ErrCode.VALIDATION
